...
- **Submodel `kind`** = membership only (`single` / `fixed-membership` / `population`). *Conditional* and *association* are **inferred**, not stored: a submodel is conditional if it contains a condition node — **any number** are allowed, AND-ed together (§12.8) — and it is an association if role arcs point into it. An association between S1 and S2 is modelled by adding a **third** submodel S3 with role arcs S1→S3 and S2→S3 — **never** S1→S2; S3 becomes the association by virtue of those arcs. (Per-record and special-grid kinds deferred.) *Checked since 2026-08-07: an association with exactly ONE role arc is reported (`association-needs-two-roles`), counting arcs rather than distinct partners so that a self-association — both roles from the same submodel, as in land-use's NEXT_TO — still passes. Necessarily deferred, since roles are drawn one at a time and every association passes through having one.*
