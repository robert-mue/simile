...
Two the models expose that are notation questions rather than bugs, and so are left open: the four **population symbols have no glyph** (they fall through to the default circle, indistinguishable from a variable and from each other), and a **submodel's kind is invisible** — `single`, `fixed-membership` and `population` all draw as the same box, though which submodels have many instances is arguably the most important thing a Simile diagram conveys.
